...
import { Plus, ChevronLeft, ChevronRight, Edit2, Check, X } from "lucide-react";
...
function FolderCover({ members, empty = false }: { members: Character[]; empty?: boolean }) {
    const preview = members.slice(0, 3);
    return (
        <div className={`char-folder ${empty ? "char-folder--empty" : ""}`} aria-hidden="true">
            <div className="char-folder__back" />
            <div className="char-folder__paper" />
            <div className="char-folder__photos">
                {preview.map((m, idx) => (
                    <div key={m.id} className={`char-folder__photo char-folder__photo--${idx}`}>
                        {m.avatar ? <img src={m.avatar} alt="" /> : <span>{(m.name || "?")[0]}</span>}
                    </div>
                ))}
                {empty && <Plus size={24} strokeWidth={1.5} />}
            </div>
            <div className="char-folder__front" />
        </div>
    );
}
...
export function WorldListView({
    ...
    // 每页 6 个位置，做六宫格
    const PAGE_SIZE = 6;
    const totalPages = Math.max(1, Math.ceil(worldGroups.length / PAGE_SIZE));
    ...
    return (
        <div className="char-view-enter flex flex-col h-full bg-[#f7f9fb] dark:bg-[#161a1e]">
            {/* 顶栏：不重叠、有安全高 */}
            <div className="char-view-header flex items-center gap-3 px-4 pb-3">
                {onClose && (
                    <button
                        type="button"
                        onClick={onClose}
                        className="char-glass-control w-11 h-11 rounded-full flex items-center justify-center text-gray-700 dark:text-gray-300 active:scale-95 shrink-0"
                        aria-label="返回上一页"
                    >
                ...
                <button
                    ...
                    className="char-glass-control flex items-center gap-1 min-h-11 px-3 rounded-full text-[#007aff] text-[12px] font-semibold active:scale-95 transition-all shrink-0"
                >
                    <Plus size={14} />
                    <span>新建世界</span>
                </button>
                        ...
                        if (item.isPlaceholder) {
                            return (
                                <button
                                    ...
                                    type="button"
                                    onClick={() => setShowNewWorld(true)}
                                    className="flex flex-col items-center text-left group min-w-0 active:scale-[0.97] transition-transform"
                                >
                                    <FolderCover members={[]} empty />
                                    <span className="text-xs font-medium text-gray-500 dark:text-gray-400 group-hover:text-[#007aff] transition-colors mt-2 px-1">
                                        {item.name}
                                    </span>
                                </button>
                            );
                                ...
                                <div className="px-1 flex items-start justify-between gap-1">
                                    ...
                                    {!isEditing && (
                                        <button
                                            ...
                                            className="char-glass-control min-w-11 min-h-11 grid place-items-center rounded-full text-gray-500 hover:text-[#007aff] transition-colors shrink-0"
                                            title="重命名"
                                        >
                                            <Edit2 size={16} />
                                        </button>
